...
// https://www.codeproject.com/Articles/5293491/Real-Time-Face-Tracking-in-the-Browser-with-Tensor
...
export class FaceMeshComponent implements OnInit {
  ...
  @ViewChild('webcamb', { static: true }) webcambElement: ElementRef;
  @ViewChild('liveView', { static: true }) liveViewbElement: ElementRef;
  @ViewChild('output', { static: true }) canvasOutput: ElementRef<HTMLCanvasElement>;
  model: any = null;
  videoWidth = 0;
  videoHeight = 0;
  children = [];
  ...
  async ngOnInit() {
    ...
    // this.toastr.info('Loading models', 'Info');
    require('@tensorflow/tfjs-backend-webgl');
    require('@tensorflow/tfjs-backend-cpu');
    // console.log(this.faceLandmarksDetection);
    // console.log(this.model);
    // this.toastr.options.positionClass = 'toast-top-center';
    // this.toastr.info('Model was loaded', 'Info');
    // this.showdata = true;
    // this.startCamera();
    // (async () => {
    ...
    // );
    this.sectionElement.nativeElement.classList.remove('invisible');

    this.cocoSsd.load().then((loadedModel) => {
    ...
    if (this.getUserMediaSupported()) {
      this.webcambElement.nativeElement.addEventListener('click', this.enableCam);
      // this.startCamera();
    } else {
  ...
  //     const webcamElement = this.videoElement.nativeElement;
  ...
  //   faces.forEach(face => {
  //     this.setText(`Face Tracking Confidence: ${face.faceInViewConfidence.toFixed(3)}`);
  ...
  //     const bWidth = x2 - x1;
  //     const bHeight = y2 - y1;
  //     this.drawLine(output, x1, y1, x2, y1);
  //     this.drawLine(output, x2, y1, x2, y2);
  //     this.drawLine(output, x1, y2, x2, y2);
  //     this.drawLine(output, x1, y1, x1, y2);
  ...
  //       this.drawTriangle(output, pointA[0], pointA[1], pointB[0], pointB[1], pointC[0], pointC[1]);
  //     }
  //   });
  ...
  predictWebcam() {
    ...
    this.model.detect(this.videoElement.nativeElement).then((predictions) => {
      ...
      //   this.liveViewbElement.nativeElement.removeChild(this.children[i]);
      // }
      ...
      //     Object.assign(highlighter.style, 'left: ' + predictions[n].bbox[0] + 'px; top: '
      //       + predictions[n].bbox[1] + 'px; width: '
      //       + predictions[n].bbox[2] + 'px; height: '
      //       + predictions[n].bbox[3] + 'px;');

      //     this.liveViewbElement.nativeElement.appendChild(highlighter);
      //     this.liveViewbElement.nativeElement.appendChild(p);
